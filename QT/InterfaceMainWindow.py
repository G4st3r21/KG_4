...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def update_positions(self):
        self.CameraPosLabel.setText(
            f"x: {self.actual_camera.position[0]}"
            f" y: {self.actual_camera.position[1]} "
            f" z: {self.actual_camera.position[2]}"
        )
        self.TargetPosLabel.setText(
            f"x: {self.actual_camera.target[0]}"
            f" y: {self.actual_camera.target[1]} "
            f" z: {self.actual_camera.target[2]}"
        )
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key.Key_D:
            self.actual_camera.move_position(np.array([-self.translation, 0, 0]))
        elif event.key() == Qt.Key.Key_A:
            self.actual_camera.move_position(np.array([self.translation, 0, 0]))
        elif event.key() == Qt.Key.Key_W:
            self.actual_camera.move_position(np.array([0, self.translation, 0]))
        elif event.key() == Qt.Key.Key_S:
            self.actual_camera.move_position(np.array([0, -self.translation, 0]))
        elif event.key() == Qt.Key.Key_Z:
            self.actual_camera.move_position(np.array([0, 0, self.translation]))
        elif event.key() == Qt.Key.Key_X:
            self.actual_camera.move_position(np.array([0, 0, -self.translation]))
        elif event.key() == Qt.Key.Key_C:
            self.actual_camera.set_position(np.array([0.0, 0.0, 100.0]))
