...
def render(qp: QPainter, polygon_quality: int, camera: Camera, mesh: Model, width, height):
    ...
    model_view_projection_matrix = np.matmul(model_view_projection_matrix, view_matrix)
    model_view_projection_matrix = np.matmul(model_view_projection_matrix, projection_matrix)
    ...
    for polygon in mesh.polygons:
        ...
        for vertex_in_polygon_ind in range(n_vertices_in_polygon):
            ...
            vertex_vecmath = np.array([vertex[0], vertex[1], vertex[2]])
        ...
        all_res_points.append((result_points, polygon.avg_dz))

    for result_points in all_res_points:
        for line_count in range(polygon_quality):
            qp.drawLine(
                int(result_points[0][line_count - 1][0]),
                int(-result_points[0][line_count - 1][1]),
                int(result_points[0][line_count][0]),
                int(-result_points[0][line_count][1]),
            )
